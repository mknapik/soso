...
(function ($, window, document, undefined) {
    ...
    $.fn.remoteChained = function (parent_selector, url, previousValue, options) {

        var settings = $.extend({}, $.fn.remoteChained.defaults, options);
        // Indicates if previous value should be used
        var previousSelectionFlag = true;
                ...
                $(this).bind("change", function () {
                    ...
                    function data_callback(json) {
                        ...
                        $(self).children().each(function () {
                            if (
                                (previousSelectionFlag && $(this).val() == previousValue) ||
                                $(this).val() === selected_key) {
                                $(this).attr("selected", "selected");
                            }
                        });
                        previousSelectionFlag = false;
                    ...
                    if(previousSelectionFlag == false) {
                        data_callback({'': ''});
                    }
                    if (entity_id != '') {
                        get_url = url + '/' + entity_id;
                        $.getJSON(get_url, data, data_callback);
                    }
                });
